...
def main():
    ...
    if df.empty:
        print("!!! No posts were scraped from Reddit. Exiting. !!!")
        return
    ...
    mentioned_df = df[df['mentioned_tickers'].str.len() > 0]
    if mentioned_df.empty:
        print("!!! No company tickers were found in any of the collected posts. !!!")
    else:
        print(f"Found tickers in {len(mentioned_df)} posts.")
        all_tickers = []
        for tickers in mentioned_df['mentioned_tickers'].dropna():
            all_tickers.extend(tickers)
        
        ticker_counts = pd.Series(all_tickers).value_counts()
        print("Top mentioned companies found in this run:")
    ...
    # Also run the export
    print("\nStep 5: Exporting data for web dashboard...")
    db.export_for_web()
    ...
    print("\nAverage Scores for posts processed in this run:")
    print("Post Scores:")
