...
async function fetchStockPrice(ticker) {
    try {
        // Use our Flask backend API to avoid CORS issues
...
async function updateStockPrices() {
    ...
    if (selectedTickers.length === 0) {
        container.innerHTML = '<div class="text-muted">Select tickers to see current stock prices</div>';
        return;
    }
    ...
    try {
        ...
        if (tickersToFetch.length > 0) {
            // Make a single API call for all uncached tickers
            ...
            fetchedResults.forEach(result => {
                stockPricesCache[result.ticker] = result;
            });
        }
        ...
        container.innerHTML = '<div class="text-danger">Error loading stock prices. Please try again.</div>';
    }
}
...
function displayStockPrices(priceData) {
    ...
    if (priceData.length === 0) {
        container.innerHTML = '<div class="text-muted">No tickers selected</div>';
        return;
    }
    
    const cards = priceData.map(data => {
        if (data.error) {
            return `
                <div class="stock-price-card neutral">
                    <div class="stock-ticker">${data.ticker}</div>
                    <div class="stock-price">Error</div>
                    <div class="stock-change neutral">Unable to fetch</div>
                </div>
            `;
        }
        ...
        const changePercentSymbol = data.changePercent > 0 ? '+' : '';
        
        return `
            <div class="stock-price-card ${changeClass}">
                <div class="stock-ticker">${data.ticker}</div>
                <div class="stock-price">$${data.price.toFixed(2)}</div>
                <div class="stock-change ${changeClass}">
                    ${changeSymbol}${data.change.toFixed(2)} (${changePercentSymbol}${data.changePercent.toFixed(2)}%)
                </div>
            </div>
        `;
    }).join('');
