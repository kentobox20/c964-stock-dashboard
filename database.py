...
class RedditDB:
    ...
    def save_daily_data(self, df, date=None):
        ...
        self._save_stock_mentions(df, conn, date)
    ...
    def _save_daily_summary(self, df, conn, date):
        ...
        for ticker in mentions_df['ticker'].unique():
            ...
            # Insert or update (replace if exists for same date/ticker)
            conn.execute('''
                INSERT OR REPLACE INTO daily_ticker_summary 
    ...
    def export_for_web(self, output_dir='docs/data'):
        ...
        sentiment_query = '''
            SELECT date, ticker, avg_overall_score 
        ...
        mentions_query = '''
            SELECT date, ticker, mention_count 
            FROM daily_ticker_summary 
            ORDER BY date, ticker
        '''
